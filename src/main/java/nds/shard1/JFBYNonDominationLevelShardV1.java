...
@ThreadSafe
@Immutable
public class JFBYNonDominationLevelShardV1<T> implements INonDominationLevel<T> {
    @Nonnull
    ...
    @Nonnull
    private volatile SortedObjectivesShardV1<IIndividual<T>, T> sortedObjectives;
     ...
     */
    public JFBYNonDominationLevelShardV1(@Nonnull JFB2014 sorter,
                                  @Nonnull List<IIndividual<T>> members) {
        this.sorter = sorter;
        if (!members.isEmpty()) {
            final int dim = members.get(0).getObjectives().length;
            this.sortedObjectives = SortedObjectivesShardV1.create(dim, members, (i, d) -> new FitnessAndCdIndividual<>(i.getObjectives(), d, i.getPayload()));
            ...
            this.sortedObjectives = SortedObjectivesShardV1.empty(0);
        }
    }

    public JFBYNonDominationLevelShardV1(@Nonnull JFB2014 sorter,
                                  @Nonnull List<IIndividual<T>> members,
                                  @Nonnull SortedObjectivesShardV1<IIndividual<T>, T> sortedObjectives) {
        this.sorter = sorter;
    ...
    @Nonnull
    public SortedObjectivesShardV1<IIndividual<T>, T> getSortedObjectives() {
...
//right
        ...
        try {
            levelLock.lock();
            final SortedObjectivesShardV1<IIndividual<T>, T> nso = sortedObjectives.update(
                addends,
                nextLevel.stream()
    ...
    @Override
    public JFBYNonDominationLevelShardV1<T> copy() {
        final List<IIndividual<T>> newMembers = new ArrayList<>(membersLeft);
        newMembers.addAll(membersRight);
        return new JFBYNonDominationLevelShardV1<>(sorter, newMembers, sortedObjectives);
    }
